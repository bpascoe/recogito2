...
trait CorpusPlacesToKMLByDescription extends BaseGeoSerializer {

  def corpusPlacesToKMLByDescription(
    docIds: Seq[String]
  )(implicit 
    ...
    val kmlFeatures = features.map { feature =>
      ...
      <Placemark>
        ...
        <description>
			{scala.xml.PCData(buildKMLStringCorpus(f, host))}
        </description>
...
// The only thing that seems to work is if ever you need CDATA, pass the required data to a method that is interpolated into the XML wrapped in the scala.xml.PCData() method.
  ...
  def buildKMLStringCorpus (f: AnnotatedPlaceFeature, host: String) : String = {
