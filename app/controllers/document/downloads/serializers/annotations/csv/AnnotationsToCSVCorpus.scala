...
trait AnnotationsToCSVCorpus extends BaseSerializer with HasCSVParsing { 
  ...
  def annotationsToCSVCorpus(docIds: Seq[String], username: String)(
    ...
    val fPlaces = entityService.listEntitiesInDocuments(docIds, Some(EntityType.PLACE))
    ...
    val fXmlStrings: Future[Seq[(UUID, String)]] = Future.sequence {  
      docIds.map { docId =>
        val part = Await.result(docService.findPartByDocAndSeqNo2(docId, 1), 1.seconds)
        if (part.getContentType == ContentType.TEXT_TEIXML.toString) 
            uploads.readTextfile(username, docId, part.getFile)
